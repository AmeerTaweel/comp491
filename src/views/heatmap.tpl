	...
	<svg viewBox="1 0 {{vw}} {{vh}}" preserveAspectRatio="xMidYMid meet" shape-rendering="crispEdges">
		...
		<g class="x-axis">
			...
			% for col in range(cols):
				% w  = round(w_map / cols, 2)
				% x  = round(size_ax_y_title + size_ax + (col * w + 0.5 * w), 2)
		...
		<g class="y-axis">
			...
			% for row in range(rows):
				...
				% x2 = round(size_ax_y_title + size_ax, 2)
			...
			% for row in range(rows):
				...
				% x = round(size_ax_y_title + 1, 2)
				% y = round(row * h + 0.5 * h + 0.5 * font_size_y_axis, 2)
				<text x="{{x}}" y="{{y}}" width="{{w - 1}}" height="{{h}}">{{y_axis[row]}}</text>
			% end
		</g>
		% end
		<!-- Map -->
